...
1. **Gate In (MANDATORY)** — Run `tas-agent memory search "<entry point or topic>"` to retrieve historical context, patterns, and related knowledge before starting.
...
7. **Gate Out (MANDATORY)** — Run `tas-agent memory store "<Feature/Module Title>" "<Key Insights and Analysis Summary>" --tags "knowledge,documentation"` to capture the new insights into semantic memory.
