...
1. **Gate In (MANDATORY)** — Run `tas-agent memory search "<topic or area of review>"` to retrieve historical review patterns and common issues in this module.
...
6. **Gate Out (MANDATORY)** — Run `tas-agent memory store "Code Review: <Feature Name>" "<Key Patterns and Lessons Learned>"` to capture review findings for future automation.
