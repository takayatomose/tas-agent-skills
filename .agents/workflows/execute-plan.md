...
2. **Search Memory** — Run `tas-agent memory search "<task/feature context>"` to find related implementation patterns or helpers.
...
6. **Capture Patterns** — If an elegant or reusable solution was implemented, run `tas-agent memory store` to save it as a pattern.
