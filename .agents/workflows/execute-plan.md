...
1. **Gate In (MANDATORY)** — Run `tas-agent memory search "<task context>"` to retrieve implementation patterns or previous task context.
...
6. **Gate Out (MANDATORY)** — Run `tas-agent memory store "Implementation Detail: <Task Name>" "<Key Patterns and Code Decisions>"` to capture technical execution into semantic memory.
