...
1. **Gate In (MANDATORY)** — Run `tas-agent memory search "<topic or area to simplify>"` to retrieve previous refactoring decisions and complexity analysis.
...
5. **Gate Out (MANDATORY)** — Run `tas-agent memory store "Simplification: <Feature Name>" "<Summary of Removed Complexity and Improved Abstractions>"` to capture lessons learned.
